...
gridData.each()//por cada Linea de Coste del Grid One Time
{
    ...
    if(pep == 'HAR' || pep == 'LIC' || pep == 'SOF' || pep == 'ENP' || pep == 'OTR'){
		...
		val_TotalOpexC = (it.y1+it.y2+it.y3+it.y4+it.y5+val_TotalOpexC)
		
    }
    if(pep == 'OPE' || pep == 'GGE' || pep == 'RAP' || pep == 'BUF'){
        log.error('Suma valores en OPEX'+it.y1)
        val_TotalOpexAnoO = it.y1+val_TotalOpexAnoO
		val_TotalOpexO = (it.y1+it.y2+it.y3+it.y4+it.y5+val_TotalOpexO)
    }
}
...
CapexSolAnoActual.updateValue(null,issue, new ModifiedValue(issue.getCustomFieldValue(CapexSolAnoActual),val_TotalOpexAnoC ),new DefaultIssueChangeHolder());

def sumaTotales = (val_TotalOpexC+val_TotalOpexO+val_TotalOpexRecSolicitado+val_TotalOpexNoItSol)
def sumaActuales = (val_TotalOpexAnoC+val_TotalOpexAnoO+val_OpexNoItSolAnoActual+val_OpexRecuSolAnoActual)

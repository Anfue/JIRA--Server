...
//def issue = ComponentAccessor.getIssueManager().getIssueObject("AJ-1097")
//IDs campos
...
/************Acceso issue CONSOLA******************* /
//def issue = ComponentAccessor.getIssueManager().getIssueObject("AJ-1067")
//IDs campos
def gridID = 18221L //  Verificacion LC finance
def gridIDPEP = 18120L //  tabla PEP
def gridIDMod = 18214L // Modificacion PEP existente
           ...
           gridDataMod.each() //por cada Linea de Modificacion PEP existentes
           {
            ...
            gridDataMod.each() //por cada Linea de Modificacion PEP existentes
            {
                if (ValEnp == it.totalAprobado && it.tipopep == 'ENP' && it.tipopep)  {
                    ID_ENP = it.nompep
                    gridDataPEP.each() //por cada Linea de PEP mete los valores
					{	
                        //log.error('it.tipopep.name-->'+it.tipopep.name)
					...
					gridData.each() //por cada Linea de Verificacion LC finance ete los valores
					{	
                        //log.error('it.tipopep.name-->'+it.tipopep.name)
                        //log.warn('ID_ENP-->'+ID_ENP)
                        //log.error('value1-->'+value1)
                        if(it.tipopep.name == 'ENP' && ID_ENP == it.IDpep){
                            log.error('VER ID-->'+it.id)
                            log.error('LC- En PEP de SOF('+Id+') tiene como ENP el '+it.IDpep +' y se pone el valor año 1 de '+value1)
                            log.error('LC- En PEP de SOF('+Id+') tiene como ENP el '+it.IDpep +' y se pone el valor año 2 de '+value2)
                            log.error('LC- En PEP de SOF('+Id+') tiene como ENP el '+it.IDpep +' y se pone el valor año 3 de '+value3)
                            log.error('LC- En PEP de SOF('+Id+') tiene como ENP el '+it.IDpep +' y se pone el valor año 4 de '+value4)
                            log.error('LC- En PEP de SOF('+Id+') tiene como ENP el '+it.IDpep +' y se pone el valor año 5 de '+value5)
                            Map < String, Object > row = new HashMap < String, Object > ();
                            row.put("y1aux", value1 != null ? value1 : 0)
                            row.put("y2aux", value2 != null ? value2 : 0)
                            row.put("y3aux", value3 != null ? value3 : 0)
                            row.put("y4aux", value4 != null ? value4 : 0)
                            row.put("y5aux", value5 != null ? value5 : 0)
                            log.error('Se modifica el ENP existente en Verificacion LC Finance')
                            gh.updateRowGrid(issue.getId(), gridID, user, row, it.id)
                        }
                    }
                }

...
//def issue = ComponentAccessor.getIssueManager().getIssueObject("DEM-5577") 
...
doAfterCreate = {
    ...
    def ClasificaciónID =17500
	def cf_clasificacion = ComponentAccessor.getCustomFieldManager().getCustomFieldObject(ClasificaciónID);
	def clasificacionValue = demanda.getCustomFieldValue(cf_clasificacion)
	def trimestreID =17501
	def cf_trimestre = ComponentAccessor.getCustomFieldManager().getCustomFieldObject(trimestreID);
	def trimestreValue = demanda.getCustomFieldValue(cf_trimestre)
	
	cf_clasificacion.updateValue(null,issue, new ModifiedValue(null, clasificacionValue ),new DefaultIssueChangeHolder());
	cf_trimestre.updateValue(null,issue, new ModifiedValue(null,  trimestreValue ),new DefaultIssueChangeHolder());
...
//campos
def cf_ano1 = ComponentAccessor.getCustomFieldManager().getCustomFieldObject(ANO1);
def cf_trimestre = ComponentAccessor.getCustomFieldManager().getCustomFieldObject(trimestre);
def cf_fini = ComponentAccessor.getCustomFieldManager().getCustomFieldObject(fini);
def cf_ffin = ComponentAccessor.getCustomFieldManager().getCustomFieldObject(ffin);
def cf_fprev = ComponentAccessor.getCustomFieldManager().getCustomFieldObject(fprev);
def cf_meses = ComponentAccessor.getCustomFieldManager().getCustomFieldObject(meses);
def cf_selProy = ComponentAccessor.getCustomFieldManager().getCustomFieldObject(selProy);


def trimestreValue = sourceIssue.getCustomFieldValue(cf_trimestre) != null ?  issue.getCustomFieldValue(cf_trimestre)?.optionId : null
def ano1Value =  (sourceIssue.getCustomFieldValue(cf_ano1) != null ? (int) sourceIssue.getCustomFieldValue(cf_ano1) : 2021   )
def mesesValue = ( sourceIssue.getCustomFieldValue(cf_meses) != null ?(int)  sourceIssue.getCustomFieldValue(cf_meses) : 12 )

switch(trimestreValue)
{   	
    case 14203:  //Q1
    	ddmm = "03/1"
    	break;
...
def fechaTexto =  ddmm+"/${ano1Value}"
def fecha =  new Date(fechaTexto);

final Calendar cal = Calendar.getInstance();
cal.setTime(fecha)//seteamos fecha de inicio
cal.add(Calendar.MONTH,mesesValue) //sumamos meses
...
if(!categoria.toString().contains("Change request")){ //NO es Change request
    ...
    issue.setCustomFieldValue(cfYear1,  sourceIssue.getCustomFieldValue(cfYear1))//MET-1094
	 doAfterCreate = {
	//DEM-1100
	 cf_fini.updateValue(null,issue, new ModifiedValue(null,fecha.toTimestamp()),new DefaultIssueChangeHolder());
     cf_ffin.updateValue(null,issue, new ModifiedValue(null,cal.getTime().toTimestamp()),new DefaultIssueChangeHolder());
	 log.debug("Entra correctament y crea PPR NO CHANGE REQUEST1")
	 }
 	 log.debug("Entra correctament y crea PPR NO CHANGE REQUEST2")
    ...
    doAfterCreate = {
		...
		//DEM-1100
		 def proyid = sourceIssue.getCustomFieldValue(cf_selProy).findAll( /\d+/ )*.toInteger()[0]
		 def proy =  ComponentAccessor.getIssueManager().getIssueObject(proyid)
		 def fproy = proy.getCustomFieldValue(cf_fini)
		 log.error(">>>>fproy: ${fproy}")
		 log.error(">>>>cal: ${cal.getTime()}")
		 
		//cf_fini.updateValue(null,issueBC, new ModifiedValue(null,cal.getTime()<fproy? cal.getTime().toTimestamp() : fproy  ),new DefaultIssueChangeHolder());
		cf_fini.updateValue(null,issueBC, new ModifiedValue(null, fproy  ),new DefaultIssueChangeHolder());
		cf_ffin.updateValue(null,issueBC, new ModifiedValue(null,cal.getTime()<fproy?  fproy : cal.getTime().toTimestamp()),new DefaultIssueChangeHolder());
		...
		def cfano1 = customFieldManager.getCustomFieldObject("customfield_12706")
		//def cfano1 = customFieldManager.getCustomFieldObject(10226L)
 ...
 log.error("Do after Create CR1")
		...
		def cfFechaPropuesta = ComponentAccessor.customFieldManager.getCustomFieldObject(14904) 
		def fechaPropuesta = proyecto.getCustomFieldValue(cfFechaPropuesta)
        ...
        log.error("Do after Create CR2")
	   
	   
	
 log.error("Do after Create CR4")


	    
	 log.error("Do after Create CR5")	
        ...
        cfano1.updateValue(null, issueBC, new ModifiedValue(issueBC.getCustomFieldValue(cfano1), proyecto.getCustomFieldValue(cfano1)), userDefaultIssue)
 ...
 log.error("Do after Create CR7")
        ...
        for(def subproyecto : subproyectos){
			 log.error("Do after Create CR iteracion ${subproyecto}")

...
//def issue = issueManager.getIssueObject("PPR-8411")
...
if(categoriaValue == 10903L || categoriaValue == 10901L){
    return
}
...
// Costes (Entornos No Productivos > 100k)
    if(	groupListManually.year1 > configMap.get("Entornos No Productivos")[4] ||
        ...
        for(row in listRows){
            if(row.tipopep.value == "SOF"){  // naturaleza del coste PPR
                def isFound = false
...
def createRowGestion(def issue, def tgeCustomFieldId, def result, def user) throws Exception{
    ...
    def y1Value = (groupListManually.year1 > configMap2.get("Horas gestion")[4] ? result.year1 * configMap2.get("Horas gestion")[5] / 100 : 0)
    def y2Value = (groupListManually.year2 > configMap2.get("Horas gestion")[4] ? result.year2 * configMap2.get("Horas gestion")[5] / 100 : 0)
    def y3Value = (groupListManually.year3 > configMap2.get("Horas gestion")[4] ? result.year3 * configMap2.get("Horas gestion")[5] / 100 : 0)
    def y4Value = (groupListManually.year4 > configMap2.get("Horas gestion")[4] ? result.year4 * configMap2.get("Horas gestion")[5] / 100 : 0)
    def y5Value = (groupListManually.year5 > configMap2.get("Horas gestion")[4] ? result.year5 * configMap2.get("Horas gestion")[5] / 100 : 0)
...
def createRow(def issue, def tgeCustomFieldId, def result, def user) throws Exception{
    ...
    def y1Value = (groupListManually.year1 > configMap.get("Entornos No Productivos")[4] ? result.calculoY1 * configMap.get("Entornos No Productivos")[5] / 100 : 0)
    def y2Value = (groupListManually.year2 > configMap.get("Entornos No Productivos")[4] ? result.calculoY2 * configMap.get("Entornos No Productivos")[5] / 100 : 0)
    def y3Value = (groupListManually.year3 > configMap.get("Entornos No Productivos")[4] ? result.calculoY3 * configMap.get("Entornos No Productivos")[5] / 100 : 0)
    def y4Value = (groupListManually.year4 > configMap.get("Entornos No Productivos")[4] ? result.calculoY4 * configMap.get("Entornos No Productivos")[5] / 100 : 0)
    def y5Value = (groupListManually.year5 > configMap.get("Entornos No Productivos")[4] ? result.calculoY5 * configMap.get("Entornos No Productivos")[5] / 100 : 0)
    ...
    row.put("unidaditAux", result.unidaditAux);
...
private getTableRows(List<Map<String, Object>> hitosGridData) {
    ...
    for (int rowNumber = 0; rowNumber < hitosGridData.size(); rowNumber++) {
        ...
        def actividadId = Long.valueOf(rowData.get("codactividad"))
        ...
        } else if(actividadIssue.getCustomFieldValue(cfActivoClarity) != "0"){
